...
export async function saveBackupConfigAction(formData: FormData) {
  ...
  if (target !== "webdav" && target !== "local") throw new Error("Ungültiges Sicherungsziel");
  const webdavUrl = value("webdavUrl");
  const localPath = value("localPath");
  if (target === "webdav" && (!/^https:\/\//i.test(webdavUrl) || !value("username"))) throw new Error("WebDAV benötigt HTTPS-URL und eigenen Wiki-Benutzer");
  if (target === "local" && (!localPath.startsWith("/") || localPath === "/" || localPath.includes(".."))) throw new Error("Ungültiger lokaler Zielpfad");
  ...
  if (target === "webdav" && !password) throw new Error("WebDAV-Kennwort fehlt");
  ...
  await writeBackupConfig({ target, webdavUrl, username: value("username"), password, localPath,
    passphrase, retentionDays, allowSelfSigned: formData.has("allowSelfSigned"), certificateFingerprint, enabled: formData.has("enabled") });
